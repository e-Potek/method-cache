...
const hijackFetch = ({ enableCaching }) => {
  ...
  cursorProto.fetch = function fetch(...args) {
    const { collectionName, selector } = this._cursorDescription;
    const isCacheEnabled = MethodCache.isCacheEnabled(enableCaching);

    if (isCacheEnabled && shouldUseCache(selector)) {
      let cache = MethodCache.getCache();

      if (!cache) {
        cache = MethodCache.initCache();
      }

      return MethodCache.getCachedValue(
        cache,
        getMergedSelector(collectionName, selector),
      );
    }
...
const initializeMethodCache = ({ enableCaching = false, log = true } = {}) => {
